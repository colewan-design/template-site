...
>
  ...
  <v-dialog v-model="modal_additional" transition="dialog-top-transition" scrollable persistent max-width="100%" enctype="multipart/form-data">
  <v-card>
    ...
    <v-col cols="13">
      <v-text-field label="Amount" type="number" v-model="additionals_crud.additional_amount" outlined hide-details dense></v-text-field>
      </v-col>
      <v-col cols="13">
...
export default {
  ...
  methods: {
      onChange(e) {
                this.additionals_crud.file = e.target.files[0];
      },
      create_additionals() {
        this.key = this.key + 1;
        this.additionals = {};
      ...
      Save(e) {
        ...
        let data = new FormData();
          data.append('file', this.additionals_crud.file);
        if(this.id) {
          axios
            .post("/Additional/additional-update/" +this.id, this.additionals_crud,config)
            .then((response) => {
              this.Additionals();
              this.close();
            })
          ...
          axios
            .post("/Additional/additional-store", this.additionals_crud,config)
            .then((response) => {
              this.Additionals();
              this.close();
            })
